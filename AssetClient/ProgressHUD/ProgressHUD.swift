...
class ProgressHUD: NSView {
    ...
    private func setupProgressIndicator() {
        ...
        case .indeterminate:
            indicator?.removeFromSuperview()
            let blurView = NSVisualEffectView(frame: NSRect(x: 0, y: 0, width: spinnerSize, height: spinnerSize))
            blurView.material = NSVisualEffectView.Material.light
    ...
    override func draw(_ rect: NSRect) {
        ...
        context.setShadow(offset: CGSize.init(width: 0, height: 0), blur: 0.1)
        switch maskType {
        case .black:
